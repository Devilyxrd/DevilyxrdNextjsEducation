 ...
 *
 * Protected route wrap'li; user her zaman var.
 *
 * Akış:
 *  1) Teslimat formu → validate
 *  2) Sepet snapshot al → ordersStore.createOrder → cart.clear
 *  3) Success ekranı 2sn → /orders'a yönlendir
 */
  ...
  if (placed) {
    return (
      <div className="mx-auto max-w-xl px-4 py-20 text-center animate-scale-in">
        ...
        <h1 className="mt-6 text-3xl font-bold text-zinc-100">Sipariş oluşturuldu!</h1>
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:py-12 lg:px-8">
      <div className="mb-8 animate-fade-in">
        <h1 className="text-3xl font-bold tracking-tight text-zinc-100 sm:text-4xl">Ödeme</h1>
      ...
      <form onSubmit={handleSubmit} className="grid gap-6 md:grid-cols-[1fr_360px]">
        <Card className="space-y-4">
          ...
          <div>
            <label className="mb-1.5 block text-sm font-medium text-zinc-300">Adres</label>
            <textarea
              value={form.address}
        ...
        <Card className="h-fit sticky top-20">
          ...
          <div className="mt-4 max-h-52 space-y-2 overflow-y-auto pr-1">
            {items.map((it) => (
              <div key={it.productId} className="flex items-center gap-2 text-sm">
                <div className="relative h-8 w-8 overflow-hidden rounded bg-zinc-800 shrink-0">
          ...
          <dl className="mt-4 space-y-2 border-t border-zinc-800 pt-4 text-sm">
            ...
            <div className="mt-3 flex justify-between border-t border-zinc-800 pt-3">
              ...
              <dd className="text-base font-bold text-brand-300">{formatTL(total)}</dd>
            </div>
          </dl>
